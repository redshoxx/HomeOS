...
export async function flushSyncQueue(): Promise<SyncResult> {
  ...
  const rows = await db.getAllAsync<QueueRow>(`SELECT * FROM sync_queue WHERE status IN ('pending','failed') ORDER BY created_at, rowid LIMIT 50`);
  let synced = 0;
  let failed = 0;
  ...
  for (const row of rows) {
    if (!allowedTables.has(row.entity_type)) {
      const message = `Nicht unterstützter Datentyp: ${row.entity_type}`;
    ...
    try {
      ...
      if (row.operation === 'DELETE') {
        const response = await supabase.from(row.entity_type).delete().eq('id', row.entity_id);
        ...
        if (row.operation === 'CREATE') {
          ...
          const response = await supabase.from(row.entity_type).upsert(payload, { onConflict: 'id' });
          error = response.error;
        } else {
          const response = await supabase.from(row.entity_type).update(payload).eq('id', row.entity_id);
          error = response.error;
        }
